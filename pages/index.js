    ...
    <main>
      ...
      <div>
        <div className="flex flex-col md:flex-row text-white hero-width w-full mx-auto relative md:-top-18 lg:-top-36 z-10">
          <div className="md:basis-[33%] bg-themeblue p-10 pr-16 space-y-5">
            ...
            <Link
              href="/services"
              className="inline-block px-6 py-3 border border-gray-300 text-xs uppercase text-white font-semibold tracking-wide"
            >
              Read More
            </Link>
          </div>
          ...
          <div className="md:basis-[33%] bg-themered p-10 pr-16 space-y-5">
            <h2 className="uppercase font-bold text-[21px] leading-tight">
              Training
            </h2>
      ...
      <div className="b-shadow p-14 bg-white hero-width mx-auto md:-mt-[1rem] lg:-mt-[7rem] mb-10">
        <div className="text-center">
          <span className="bg-themeblue text-white p-2 uppercase mb-4 inline-block text-xs">
            Story About Us
          </span>
          <h2
            className={`text-themered uppercase text-[32px] font-black leading-tight`}
          >
            PUBLIC HEALTH | RESEARCH & <br />
            TRAINING.
          </h2>
          <div className="text-sm">
            <ReactMarkdown className="w-[90%] text-black md:w-3/4 mx-auto mt-8 mb-6 text-justify flex flex-col space-y-6">
              {homepageData?.home.phearts_story_summary}
            </ReactMarkdown>
          ...
          <Link
            href="/about/history"
            className="uppercase text-xs text-themered font-bold tracking-wide"
          >
            Learn More...
          </Link>
        </div>
      ...
      <div className="py-10 bg-themered">
        <div className="hero-width w-full mx-auto flex flex-col justify-center text-center lg:text-left items-center lg:flex-row lg:justify-between space-y-4 lg:space-y-0">
          <div>
            <h3 className="font-subheading text-white">
              REQUEST FOR APPOINTMENT
            </h3>
            <p className="text-white text-sm">
              Provide us your details to visit a doctor as soon as possible
            </p>
          </div>
          <Link
            className="px-6 py-3 border text-white border-gray-300 text-xs uppercase font-semibold tracking-wide"
            href="/appointment"
          >
      ...
      <Stories
        theme={"dark"}
        testimonialData={homepageData?.home?.testimonial}
      />
...
export const getStaticProps = async () => {
  const homeRes = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/home?populate=hero_slider.image&populate=testimonial.client_photo&populate=collaborations.company_photo`
  );
  const homeApidata = await homeRes.json();
  console.log("Home Api Data", homeApidata);
  const newsRes = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/newses?sort=news_date:desc&pagination[start]=0&pagination[limit]=5&populate=*`
  );
  const newsApidata = await newsRes.json();
  const eventRes = await fetch(
    `${
      process.env.NEXT_PUBLIC_BASE_URL
    }/events?filters[start_date][$gte]=${new Date().toISOString()}&sort=start_date&pagination[start]=0&pagination[limit]=3`
  );
  const eventApidata = await eventRes.json();
  const departmentsRes = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/services/1?populate=departments.photo`
  );
  const departmentApidata = await departmentsRes.json();
  const albumsRes = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/gallery-albums?populate=*`
  );
