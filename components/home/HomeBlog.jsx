    ...
    <SwiperSlide key={id} className="h-[300px]">
      <div className="relative bg-[#F8F9F9]">
        ...
        <div className="p-3 bg-white w-[95%] mx-auto relative -mt-5">
          ...
          <h4
            className={`text-lg text-themeblue leading-snug font-extrabold ${poppinsBold.variable} font-poppins mt-2`}
          >
            {title?.slice(0, 45) + (title?.length > 45 ? "..." : "")}
          </h4>
          <p className="text-blackshadow text-sm mt-2 mb-2">
            {summary?.substring(0, 100)}..
          </p>
          <Link
            href={`/news/${id}`}
            className="text-sm font-semibold text-themegreen"
          >
            Read More
          </Link>
        </div>
  ...
  return (
    <div className="pt-24 pb-32">
      <div className="hero-width w-full mx-auto text-center">
        ...
        <h2 className="uppercase text-[32px] font-black leading-tight mb-14">
          Latest News
        </h2>
        <div>
          <Swiper
          ...
          >
            ...
            {newsItems.map((item) => {
              const { preview_img, news_date, title, summary } =
                item?.attributes;
              return causesSlide({
                id: item?.id,
                image: preview_img,
        ...
        <Link
          href="/news"
          className="text-sm font-semibold text-themegreen border-2 border-themegreen py-2 px-4 inline-block mt-6"
        >
          View All News
        </Link>
      </div>
